 ...
 */
public class Greedy_1947_ver2 {

	static int T, N;
	static int[] person; //index : 서류 성적, value : 면접 성적 (순위) 
	static int ans;
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int t = 1; t <= T; t++) {
			...
			for (int i = 0; i < N; i++) {
				...
				int documentScore = Integer.parseInt(st.nextToken())-1;
				int interviewScore = Integer.parseInt(st.nextToken())-1;
				person[documentScore] = interviewScore;
			}
			int min = person[0];
			ans = 1;
			for (int i = 1; i < N; i++) {
